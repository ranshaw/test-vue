<template>
    <div class="commentOn">
        <div class="noContent" style="display: none" v-show="commentList.length == 0"  >暂无留言</div>
        <div class="messList" id="div_list" v-for="item in commentList">
            <div class="reply">
        ...
        <div class="page" >
            <a href="javascript:;" :class="{active:isActive == n}" @click="getList(n)"  v-for="n in pages">{{n}}</a>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            getList:function (n) {
                ...
                this.isActive = n ;
                this.ajax.get(n).then(function (res) {

                    if(res.data.code === 0) {
                        me.commentList = res.data.info.rows;
                        me.pages =  Math.ceil(res.data.info.total/5);
                    }
                }).catch(function (err) {
                    console.log(err)
                })
            ...
            delComment:function (item) {
                ...
                this.ajax.del(item._id).then(function (res) {

                    if(res.data.code === 0) {
                        me.getList(me.isActive);

                        if(me.commentList.length === 1) {
                            me.pages = me.pages - 1 ;
                            me.isActive = me.isActive - 1;
                            me.getList(me.isActive);
                        }
                    }
                    console.log(res)
        ...
        mounted () {
            ...
            this.newComment.$on('refresh',function(){
